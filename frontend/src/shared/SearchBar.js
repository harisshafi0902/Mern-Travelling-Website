...
const SearchBar = () => {
  ...
  const searchHandler = async () => {
    ...
    const groupsize = maxGroupSizeRef.current.value;
    ...
    const res = await fetch(
      `${BASE_URL}/tour/search/getTour/getTourBySearch?city=${location}&distance=
      ${distance}&maxGroupSize=${groupsize}`
    );
    const result = await res.json();

    navigate(
      `tours/search?city=${location}&distance=
    ${distance}&maxGroupSize=${groupsize}`,
      { state: result.data }
    );
  };
